...
import { aboutMetrics, aboutBio } from '../data/about';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <motion.div variants={fadeUpVariants} className="relative mb-6 select-none">
              <h2
                className="text-[2.15rem] sm:text-6xl md:text-7xl lg:text-[5.4rem] tracking-tight uppercase leading-[1] sm:leading-[0.88] break-words"
            ...
            <motion.p
              ...
              style={{ fontFamily: "'Montserrat', sans-serif" }}
            >
              I'm <span className="text-[#F3DBB3] font-medium">{profile.name}</span>, an {profile.title} based in{' '}
              {profile.location}. {aboutBio}
            </motion.p>

            {/* Metrics Grid */}
            <motion.div
              variants={fadeUpVariants}
              className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-x-6 gap-y-8 pt-6 pb-2 border-t border-[#8C6D4F]/25"
            >
              {aboutMetrics.map((metric, idx) => (
                <div key={metric.label} className="flex flex-col">
                  <span
                    className={`text-2xl sm:text-3xl lg:text-[1.9rem] font-light tracking-tight ${
                      idx % 2 === 0 ? 'text-[#F4EBE2]' : 'text-[#D4AF37]'
                    }`}
                    style={{ fontFamily: "'Bebas Neue', sans-serif" }}
                  >
                    {metric.value}
          ...
          <div className="lg:col-span-5 flex items-center justify-center relative perspective-[1400px]">
            ...
            >
              ...
              <div className="relative overflow-hidden w-full max-w-[390px] aspect-[4/5] bg-black rounded-sm">
                <img
                  src={aboutImg}
                  alt={`${profile.name}, ${profile.title}, working at his desk`}
