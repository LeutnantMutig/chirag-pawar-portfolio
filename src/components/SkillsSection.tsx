...
import { skillBlocks, learningSkills } from '../data/skills';
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        ...
        >
          <h2
            className="text-[2.15rem] sm:text-6xl md:text-7xl lg:text-[5.5rem] tracking-tight uppercase leading-[1] sm:leading-[0.85] break-words select-none"
          ...
          {skillBlocks.map((block, idx) => (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4 gap-3">
                ...
                <span className="text-[10px] font-mono px-2.5 py-0.5 border border-[#8C6D4F]/40 text-[#C4B5A5] bg-[#17130F] group-hover:border-[#D4AF37]/50 group-hover:text-white transition-all text-right">
                  {block.stat}
                </span>
              </div>

              <h3
                className="text-2xl sm:text-4xl font-normal tracking-wide text-white mb-3 group-hover:text-[#F7E7C4] transition-colors break-words"
                style={{ fontFamily: "'Bebas Neue', sans-serif" }}
              >
                {block.title}
              </h3>
        ...
        {/* Learning-Stage Honesty Strip */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8, delay: 0.2 }}
          className="mt-8 p-6 sm:p-7 rounded-sm border border-[#8C6D4F]/25 bg-[#0C0A08]/70"
        >
          <span
            className="block text-[10px] font-mono tracking-[0.25em] uppercase text-[#8C6D4F] mb-4"
            style={{ fontFamily: "'Montserrat', sans-serif" }}
          >
          ...
          <div className="flex flex-wrap gap-3">
            {learningSkills.map((skill) => (
              <span
                key={skill.name}
                className="inline-flex items-center gap-2 px-3.5 py-1.5 text-[10.5px] font-medium tracking-[0.16em] uppercase rounded-sm border border-dashed border-[#8C6D4F]/50 text-[#A8988B]"
                style={{ fontFamily: "'Montserrat', sans-serif" }}
              >
                {skill.name}
                <span className="text-[9px] text-[#D4AF37]/80 normal-case tracking-normal">{skill.note}</span>
              </span>
            ))}
          </div>
        </motion.div>
